...
export class ProductService {
  ...
  public async updateProduct(productId: string, dto: ProductDto): Promise<Product> {
  ...
  public async findProduct(dto: FindProductDto): Promise<ProductAggregateModel> {
    const product = await this.productModel
      ...
      .lookup({
        ...
        pipeline: [{ $match: { $expr: { $eq: ["$productId", "$$searchId"] } } }],
        as: "reviews",
      })
      .exec();
    if (!product.length) {
