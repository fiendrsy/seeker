...
export class ReviewService {
  ...
  public async findAllReviewByProductId(productId: string): Promise<ReviewDocument[]> {
  ...
  public async deleteReview(reviewId: string): Promise<ReviewDocument> {
    const delReview = await this.reviewModel.findOneAndDelete({ reviewId }).exec();
    if (!delReview) {
      throw new HttpException(NOT_FOUND, HttpStatus.NOT_FOUND);
    }
    return delReview;
  ...
  public async deleteAllReviewByProductId(productId: string): Promise<DeleteResult> {
