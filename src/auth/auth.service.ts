...
export class AuthService {
	...
	public async signIn(dto: SignInDto): Promise<UserProfile & Tokens> {
		const user = await this.authValidationService.validateUser(
			dto.login,
			dto.password,
		);
		...
		await this.authTokensService.updateRefreshTokenHash(
			user._id,
			tokens.refreshToken,
		);
		return {
	...
	public async refreshToken(userId: string, token: string): Promise<Tokens> {
		...
		await this.authValidationService.validateTokens(
			token,
			user.refreshTokenHash,
		);
		const tokens = this.authTokensService.generateTokens(
			user._id,
			user.login,
			user.roles,
		);
